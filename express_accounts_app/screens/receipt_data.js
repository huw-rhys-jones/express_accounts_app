...
export default function ReceiptData({route, navigation }) {
  ...
  const createExpenseObject = () => {
    const payload =  {
      ...
      date: date,    // Store date in ISO format
      category: category
    };
    ...
    {images.map((image) => 
      <TouchableOpacity onPress={() => setModalVisible(true)} style={receipt.receiptImageBox}>
        <Image
                source={{uri : image}}
                contentFit="contain"
                style={{ height: 175, aspectRatio: 0.5 }}
              />
      </TouchableOpacity>
    ...
    <ScrollView contentContainerStyle={logoStyles.scrollContainer}>
      ...
      <View style={receipt.container}>
        ...
        <View style={receipt.buttonsBottom}>
          ...
          <TouchableOpacity onPress={createExpenseObject} style={receipt.button}>
